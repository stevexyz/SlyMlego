...
if os.path.isfile(Const.MODELFILE+".hdf5"):
    ...
    def neck(nip,nop):
        def unit(x):
            ...
            if nip==nop:
                ...
                x = relu(x)
                x = Convolution2D(nbp,(1,1))(x)
                x = BatchNormalization(axis=-1)(x)
                x = relu(x)
                x = Convolution2D(nbp,(3,3),padding='same')(x)
                x = BatchNormalization(axis=-1)(x)
                x = relu(x)
    ...
    net = inp
    net = Convolution2D(64*Const.NUMFEATURES,(3,3),padding='same')(net)
    net = cake(64*Const.NUMFEATURES,64*Const.NUMFEATURES,2)(net)
    net = cake(64*Const.NUMFEATURES,64*Const.NUMFEATURES,3)(net)
    net = cake(64*Const.NUMFEATURES,64*Const.NUMFEATURES,4)(net)
